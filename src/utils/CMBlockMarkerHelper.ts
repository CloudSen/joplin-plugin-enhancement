...
export class CMBlockMarkerHelper {
    ...
    private _markRanges(blockRangeList) {
        for (const blockRange of blockRangeList) {
            ...
            if (cursor.line < from.line || cursor.line > to.line
                ...
                // Keep a regular text marker instead of a replacement widget.
                // Joplin's CM6 compatibility layer positions replacement
                // widgets relative to the rendered SVG, which can overlay the
                // rightmost part of a Mermaid label. The marker class still
                // hides the source lines and preserves the edit operation.
                ...
                this.setStyleAndLogical(doc, from, to, textMarker, wrapper, lineWidget);
    ...
    private clearMarkerLineWidget(marker) {
        // check whether there exists rendered line widget
        const line = this.editor.lineInfo(marker.find().to.line);
        if (line.widgets) {
            for (const wid of line.widgets) {
                if (wid.className === this.MARKER_CLASS_NAME + '-line-widget') {
                    wid.clear();
